...
namespace CarBooking.WebUI.Areas.Admin.Controllers
{
	...
	public class AdminPricingController : Controller
	{
		...
		[HttpPost]
		[Route("CreatePricing")]
		public async Task<IActionResult> CreatePricing([FromForm] CreatePricingDto PricingDto)
		{
			var client = _httpClientFactory.CreateClient();
		...
		public async Task<IActionResult> UpdatePricing(UpdatePricingDto updatePricingDto)
		{
			...
			var responseMessage = await client.PutAsync($"https://localhost:7182/api/Pricings/", content);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index", "AdminPricing", new { area = "Admin" });
			}
